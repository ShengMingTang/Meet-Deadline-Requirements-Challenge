...
def generateBackground(dur: int, totalTime: float, num: int):
    ...
    for p in (Path('..')/'datasets'/'background_traffic_traces').glob('*.csv'):
        ...
        data.extend(diff(d))
    data = np.array(data)
    np.savetxt('test.txt', data, fmt='%.4f')
    ...
    for i in range(num):
        ...
        gen = np.array(gen)
        np.savetxt(dir/f'{i}.csv', gen, fmt='%.4f')
...
def generateScenarios(dur: int, totalTime: float, num: int):
    ...
    for i in range(num):
        ...
        for j in range(len(dataPri)):
            ...
            np.savetxt(bdir/f'block-priority-{j}.csv', gen, fmt='%.4f')
        # nw
        gen = genByTime(dataNw, totalTime, dur)
        np.savetxt(str(ndir/f'traces_0.txt'), gen, fmt='%.4f')
